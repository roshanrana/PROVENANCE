...
| **Stack** | Python 3.12, uv, pytest, ruff, mypy, NumPy/SciPy, cryptography/ed25519, Go, vLLM, llm-d, Kubernetes/kind, Helm-style manifests. |
| **Validation** | `make check`, 200+ tests, coverage gates, `make attest-demo`, receipt tamper tests, bootstrap/permutation/AUC tests, Go salt-derivation tests, CI mirror of local gates. Hardware-dependent vLLM measurements and kind cluster verification are documented as explicit next gates. |
