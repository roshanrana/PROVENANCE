...
| D-06 | **Pin prefix caching off for ATTEST's primary claim *on vLLM***; document the APC × batch-invariance interaction as a secondary finding. **Amended by ADR-009:** the pin is a property of vLLM's upstream integration gap, not of determinism. SGLang runs deterministically with its radix cache on (FA3/Triton), and is measured that way. | The two features are not integrated in vLLM. A determinism claim with APC in an unknown state is not a claim — but the non-composition is itself a result, and stating it as a fact about *determinism* rather than about *vLLM* would have been wrong. |
...
| D-08 | **Receipts bind the *resolved* engine configuration**, read back from the engine — not the flags the operator intended to pass. Applies to both engines (ADR-009). | vLLM's `override_envs_for_invariance()` mutates the environment; SGLang can resolve a different attention backend or silently lose the radix cache. Intended ≠ actual, on either. |
...
| FR-A-08 | Document the caching × determinism interaction, **decomposed across engines**. | Secondary experiment and writeup section establishing whether the two compose and what a deployment must give up to have both. On vLLM they do not compose, so the cost of determinism and the cost of losing the cache are confounded; the SGLang arm separates them (ADR-009). Acceptance requires the 2×2 — caching × determinism — with both costs reported separately, or a stated reason why a cell could not be produced. (D-06) |
